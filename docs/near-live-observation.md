...
The bounded archive follow-up completed the installed-plugin test on 2026-09-05 with Codex CLI 0.153.4. The versioned plugin cache carried the NativeAOT executable byte-for-byte, the installed skill derived its root without a hard-coded machine path, and the executable launched in place without PATH installation or Windows elevation. Both a first and a subsequent command started inside the sandbox, but their `%LOCALAPPDATA%` marker writes were denied; automatic approval was required separately for each successful current-user run. PF1 is therefore **PF1-C — Recurring approval** on the tested version. See [experiment 004](../experiments/004-plugin-native-companion/README.md) and its [sanitized evidence](evidence/pf1-plugin-launch.json).
