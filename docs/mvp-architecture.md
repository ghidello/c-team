...
For MVP, returned execution data is scoped to the exact caller's activated filesystem project root. A proven parent/session relationship may identify a mission root outside that root, but does not activate another project automatically.

Before reading a relative's execution records, resolve its own project using the same bounded identity and project-normalization rules. By default, include its activity and usage only when that normalized root equals the caller's activated root. Different filesystem roots—including sibling worktrees and nested repositories—are therefore excluded by default in the MVP rather than assumed equivalent merely because they share Git history or contain C-Team markers. Recognize both `.git` files and directories as filesystem boundaries.

This root-path rule is a conservative MVP boundary, not the long-term definition of C-Team project identity. A future worktree-aware design may admit multiple filesystem roots into one logical C-Team project only through an explicit, validated equivalence signal—for example a stable C-Team project identity combined with proven parent/child mission relationship and verified Git common-repository identity. Do not infer that equivalence from Git history, branch names, origin URLs, matching `.cteam` contents, or proximity alone.

Represent an out-of-scope or unresolved relative as an excluded branch using only relationship identity already established from admitted evidence. Omit its paths, prompts, activity, and usage, mark coverage partial, and do not traverse through that branch. A child whose root is excluded remains displayed as the exact caller within a partial mission view. Cross-root observation, including worktree equivalence, is deferred to a separately designed opt-in/logical-project contract.
